...
def test_event_display_draws_tile_5_iog_6_edge_in_3d_and_2d():
    ...
    assert len(trace_3d.x) == 36  # 12 box edges, separated by None values
    assert min(value for value in trace_3d.x if value is not None) < tile_box.zmin
    assert max(value for value in trace_3d.x if value is not None) > tile_box.zmax
